...
namespace Crystal
{
	public class WorldMethods
	{
        public static void RoundHill(int X, int Y, int Xmult, int Ymult, int strength, bool initialplace, ushort type)
        {
            if (initialplace)
            {
                WorldMethods.TileRunner(X, Y, (double)strength * 5.5f, 1, type, false, 0f, 0f, true, true);
            }
            for (int rotation2 = 0; rotation2 < 350; rotation2++)
            {
                ...
                WorldGen.TileRunner(X + DistX, Y + DistY, (double)strength, 1, type, false, 0f, 0f, true, true);
            }
        }
